...
define sequence condition_hint_1
	set flush_keyboard yes
	set description "Führt mehrere Items nacheinander aus"
	run learning_session_setup_1 always
	run while_there_is_time_left_1 always
	run feedback_text_1 always
	run new_feedback_2 always
	run save_data_1 always
...
define sequence control_1
	set flush_keyboard yes
	set description "Führt mehrere Items nacheinander aus"
	run slimstampen_setup_c_1 always
	run instructions_c_1 always
	run learning_session_setup_c_1 always
	run while_there_is_time_left_c_1 always
	run save_data_c_1 always
...
define sequence experiment_loop_odd
	set flush_keyboard yes
	set description "Führt mehrere Items nacheinander aus"
	run control_1 always
	run test_1 always
	run Break_1 always
	run condition_hint_1 always
	run test_1 always
...
define inline_script feedback_text_1
	set description "Führt Python Code aus"
	___run__
	acc = 100.*self.get('total_correct')/self.get('total_responses')
...
define sketchpad instructions_c_1
	set duration keypress
	set description "Displays stimuli"
	draw textline center=1 color=white font_bold=no font_family=mono font_italic=no font_size=18 html=yes show_if=always text="Please type the corresponding translation to the english word." x=0 y=0 z_index=0
...
define inline_script learning_session_setup_1
	set description "Executes Python code"
	___run__
	# Start the clock
	var.session_start_time = clock.time()
...
define inline_script learning_session_setup_c_1
	set description "Executes Python code"
	___run__
	# Start the clock
	var.session_start_time = clock.time()
...
define feedback new_feedback_1_1
	set reset_variables yes
	set duration keypress
	set description "Gibt Versuchspersonen Feedback"
	draw textline center=1 color=white font_bold=no font_family=mono font_italic=no font_size=20 html=yes show_if=always text="[feedback_test] You have [correct_test] from [responses_test] possible answers correct. That corresponds to an accuracy of [accuracy_test]%." x=0 y=0 z_index=0
...
define sequence new_sequence_2
	set flush_keyboard yes
	set description "Führt mehrere Items nacheinander aus"
	run present_trial_testing_2 always
...
define inline_script present_trial_c_1
	set description "Executes Python code"
	___run__
	trial_start_time = clock.time()
...
define inline_script present_trial_testing_2
	set description "Executes Python code"
	___run__
	trial_start_test = clock.time()
...
define inline_script save_data_1
	set description "Executes Python code"
	___run__
	# Write the SlimStampen data to the OpenSesame log file
	dat = m.export_data()
...
define inline_script save_data_c_1
	set description "Executes Python code"
	___run__
	# Write the SlimStampen data to the OpenSesame log file
	dat = m.export_data()
...
define inline_script slimstampen_setup_c_1
	set description "Executes Python code"
	___run__
	m = SpacingModel()
...
define sequence test_1
	set flush_keyboard yes
	set description "Führt mehrere Items nacheinander aus"
	run test_instructions_1 always
	run new_inline_script_1 always
	run new_loop_1 always
	run test_feedback_1 always
	run new_feedback_1_1 always
...
define inline_script test_feedback
	...
	list_of_tuples = list(zip(word, translation, cor, rt_word)) 
	...
	# pandas Dataframe.  
	df_test = pd.DataFrame(list_of_tuples, columns = ['english', 'translation','correct', 'rt'])
...
define inline_script test_feedback_1
	set description "Führt Python Code aus"
	___run__
	acc_test = 100.*self.get('correct_test')/self.get('responses_test')
...
define sketchpad test_instructions_1
	set duration keypress
	set description "Präsentiert Stimuli"
	draw textline center=1 color=white font_bold=no font_family=mono font_italic=no font_size=20 html=yes show_if=always text="In the following section your learning progress will be tested. Please type the corresponding translation." x=0 y=0 z_index=0
...
define sequence trial_sequence_c_1
	set flush_keyboard yes
	set description "Runs a number of items in sequence"
	run present_trial_c_1 always
...
define loop while_there_is_time_left_c_1
	set source table
	set repeat 1000
	set order random
	set description "Repeatedly runs another item"
